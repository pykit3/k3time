...
class TestTimeutil(unittest.TestCase):
    ...
    def test_direct_format(self):
        ...
        for fmt_key, tm_str in list(test_case['format'].items()):
            ...
            self.assertEqual(tm_str,
                             k3time.format(dt_key,
                                             k3time.formats[fmt_key]))

            # format_ts
            now = int(time.time())
            self.assertEqual(k3time.format_ts(now, fmt_key),
                             k3time.format_ts(now, k3time.formats[fmt_key]))
    ...
    def test_format_ts(self):
        ...
        for fmt_key, tm_str in list(test_case['format'].items()):
            ...
            convert_tm_str = \
                k3time.format_ts(test_case['ts']['second_accuracy'], fmt_key)
    ...
    def test_to_sec(self):
        ...
        for inp in cases:
            ...
            self.assertEqual(ts, k3time.to_sec(inp),
                             'convert {inp} to second'.format(inp=repr(inp)))

            self.assertEqual(ts, k3time.to_sec(str(inp)),
                             'convert {inp} to second'.format(inp=repr(inp)))
